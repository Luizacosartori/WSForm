...
router.post("/getStaff/", (request, response) => {
    connection.query(
        "SELECT accesstoken FROM currentuser WHERE username = '" +
        request.body.username +
        "'",
        (err, records, fields) => {
            if (err) {
    ...
    connection.query(
        ...
        (err, records, fields) => {
            if (err) {
                ...
                mindbodyAPI.getTreatments(
                    records[0].accesstoken,
                    function (treatments) {
                        for (
                            let index = 0;
                            index < treatments.Appointments.length / 20;
                            index++
                        ) {
                            mindbodyAPI.getClient(
                                records[0].accesstoken,
                                treatments.Appointments.slice(index * 20, index * 20 + 20),
                                function (client) {
                                    client.Clients.forEach((c) => {
                                        connection.query(
                                            ...
                                            (err) => {
                                                if (err) {
                                                    console.log(
                                                        "Error when inserting the client data",
                                                        err
                                                    );
                                                }
                                            }
...
router.get("/treatment/", (request, response) => {
    ...
    connection.query(
        'SELECT * from `treatment` where date(`treatment_StartDateTime`) ="' +
        newDate +
...
router.get("/massageForm/", (request, response) => {
    connection.query(
        "SELECT * FROM client_massage_form ",
        (err, records, fields) => {
            if (err) {
                console.log("Error when retriving the data");
            } else {
...
router.get("/:id", (req, res) => {
    connection.query(
        "SELECT * FROM client_massage_form  WHERE client_id=" + req.params.id,
        (err, records, fields) => {
            if (err) {
...
router.get("/staff/:id", (request, response) => {
    connection.query("SELECT staff.full_name, treatment.treatment_id FROM staff INNER JOIN treatment ON treatment.staff_id = staff.staff_id WHERE treatment_id =" + request.params.id,
        (err, records, fields) => {
            if (err) {
                console.log(err);
            } else {
...
router.post("/NewClientMassageForm/", (req, res) => {
    ...
    var hear_about_us_healthcare_provider =
        req.body.hear_about_us_healthcare_provider;
    var hear_about_us_online_advertisement =
        req.body.hear_about_us_online_advertisement;
